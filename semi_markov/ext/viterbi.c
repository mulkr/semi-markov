...
VIT_PREFIX void viterbi(const double* trans_mat,
    ...
    for(size_t i=0;i<state_num;++i){
        ...
        for(size_t j=0;j<max_dur;++j){
            sum += dur_probs[index(j,i,max_dur)];
        }
        dur_sum[i] = sum;
    }
    ...
    for(size_t i=0;i<state_num;++i)
        delta[index(0,i,pad_len)] = log(init_state[i]) + log(obs_probs[index(0,i,time_len)]);
    ...
    for(size_t t=1; t<pad_len; ++t){
        for(size_t state=0; state<state_num; ++state){
            for(size_t dur=1; dur<=max_dur; ++dur){
                ...
                size_t max_index = 0;
                for(size_t i=0;i<state_num;++i){
                    double val = delta[index(start,i,pad_len)]+log(trans_mat[index(state,i,state_num)]);
                    if(val>max_delta){
                        max_delta=val;
                        max_index=i; 
                    }
                ...
                for(size_t i=start;i<end;++i){
                    probs *= obs_probs[index(state,i,state_num)];
                }
                ...
                double delta_temp = max_delta + emit_probs + log(dur_probs[index(dur,state,max_dur)]/ dur_sum[state]);

                if(delta_temp>delta[index(t,state,pad_len)]){
                    delta[index(t,state,pad_len)] = delta_temp;
                    psi[index(t,state,pad_len)] = max_index;
                    psi_dur[index(t,state,pad_len)] = dur;
    ...
    for(size_t i=time_len+1;i<pad_len;++i)
        for(size_t j=0;j<state_num;++j)
            if(delta[index(i,j,pad_len)]>delta[index(state_time_after,j,pad_len)])
                state_time_after = i;
    //select state from delta at state_time_after where maximal value
    int state_after = 0;
    for(size_t i=0;i<state_num;++i)
        if (delta[index(state_time_after,i,pad_len)]>delta[index(state_time_after,state_after,pad_len)])
            state_after = i;
    
    //backtrack
    for(size_t i=0;i<pad_len;++i){
        for(size_t j=0;j<state_num;++j){
            printf("%zu ",psi_dur[index(i,j,pad_len)]);
        }
        printf("\n");
    }
    ...
    printf("state: %d\n",state_after);
    while(time_loc>1){
        int dstar = psi_dur[index(time_loc,state_after,pad_len)];
        if(dstar == 0){
            printf("time_loc: %d\n",time_loc);
            printf("state: %d\n",state_after);
            printf("LOOP!\n");
        ...
        state_after = psi[index(time_loc,state_after,pad_len)];
